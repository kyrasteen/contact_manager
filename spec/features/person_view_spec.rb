...
describe 'the contact view', type: :feature do
    let(:contact) { Person.create(first_name: 'John', last_name: 'Doe') }
  describe 'phone_numbers', type: :feature do

    before(:each) do
      contact.phone_numbers.create(number: "555-1234", contact_id: contact.id, contact_type:'Person' )
      contact.phone_numbers.create(number: "555-3254", contact_id: contact.id, contact_type:'Person')
      visit person_path(contact)
    end
    ...
    it "has a link to add a new phone number" do
      expect(page).to have_link('Add Phone Number', href: new_phone_number_path(contact_id: contact.id, contact_type: 'Person'))
    end
    ...
    it "adds a new phone number" do
      ...
      expect(current_path).to eq(person_path(contact))
  ...
  describe 'email_addresses', type: :feature do
    ...
    before(:each) do
      contact.email_addresses.create(address:'myadd@example.com', contact_id: contact.id, contact_type: 'Person')
      contact.email_addresses.create(address:'diffadd@example.com', contact_id: contact.id, contact_type: 'Person')
      visit person_path(contact)
    end
    ...
    it 'adds an email address' do
      ...
      expect(current_url).to eq('http://www.example.com/email_addresses/new?contact_id=1&contact_type=Person')
